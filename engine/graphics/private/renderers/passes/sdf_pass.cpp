...
struct sdf_mark_dirty_pages_cs : public shader_cs
{
    ...
    struct constant_data
    {
        std::array<sdf_pass::clipmap, SDF_CLIPMAP_LEVEL_COUNT> clipmaps;
...
void sdf_pass::add(render_graph& graph, const parameter& parameter)
{
    ...
    initialize_clipmaps(graph, context.get_camera().position, clipmap.need_clear);
...
void sdf_pass::initialize_clipmaps(render_graph& graph, const vec3f& camera_position, bool clear)
{
    float page_extent = SDF_CLIPMAP_PAGE_EXTENT;
    for (std::uint32_t level = 0; level < SDF_CLIPMAP_LEVEL_COUNT; ++level)
    {
        vec3f page_coord = vector::floor(camera_position / page_extent);
        page_coord -= static_cast<float>(SDF_CLIPMAP_PAGE_COUNT_PER_AXIS) / 2.0f;

        auto& clipmap = m_clipmaps[level];
        clipmap.position = page_coord * page_extent;
        clipmap.extent = page_extent * SDF_CLIPMAP_PAGE_COUNT_PER_AXIS;
    ...
    if (clear)
    {
        ...
        graph.add_pass<pass_data>(
            ...
            {
                data.clipmap_state =
                    pass.add_buffer_uav(m_clipmap_state, RHI_PIPELINE_STAGE_COMPUTE);
...
void sdf_pass::mark_invalidated_page(render_graph& graph)
{
    ...
    graph.add_pass<pass_data>(
        ...
        [clipmaps = m_clipmaps](const pass_data& data, rdg_command& command)
        {
            auto& device = render_device::instance();
...
void sdf_pass::mesh_grid_cull(render_graph& graph)
{
    ...
    graph.add_pass<calculate_offset_pass_data>(
        ...
        [clipmaps = m_clipmaps](const calculate_offset_pass_data& data, rdg_command& command)
        {
            auto& device = render_device::instance();
    ...
    graph.add_pass<build_list_pass_data>(
        ...
        [clipmaps = m_clipmaps](const build_list_pass_data& data, rdg_command& command)
        {
            auto& device = render_device::instance();

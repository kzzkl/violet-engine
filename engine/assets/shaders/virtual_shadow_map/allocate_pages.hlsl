...
groupshared uint gs_render_physical_page_list_offset;
...
void cs_main(uint3 dtid : SV_DispatchThreadID, uint group_index : SV_GroupIndex)
{
    ...
    if (group_index == 0)
    {
        ...
        InterlockedAdd(render_physical_page_texels_indirect_args[0].z, gs_allocate_page_count, gs_render_physical_page_list_offset);
    }
    ...
    {
        RWStructuredBuffer<uint> render_physical_page_list = ResourceDescriptorHeap[constant.render_physical_page_list];
        render_physical_page_list[gs_render_physical_page_list_offset + group_index] = gs_allocate_page_index[group_index];
    }

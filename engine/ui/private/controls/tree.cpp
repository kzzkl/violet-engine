      ...
      m_highlight_color(theme.highlight_color)
{
    ...
    m_top->event()->on_mouse_press = [this](window::mouse_key key, int x, int y) {
    ...
    m_button->hide();
    m_button->event()->on_mouse_press = [this](window::mouse_key key, int x, int y) {
    ...
    : tree_node(name, theme)
{
    ...
    m_icon->layout()->set_margin(theme.icon_font->heigth() * theme.icon_scale * 0.5f, LAYOUT_EDGE_RIGHT);
...
void tree_node::add_node(tree_node* child)
{
    ...
    child->m_top->layout()->set_padding(m_top->layout()->get_padding(LAYOUT_EDGE_LEFT) + m_padding_increment, LAYOUT_EDGE_LEFT);

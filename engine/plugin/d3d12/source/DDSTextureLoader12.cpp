...
inline void SetDebugObjectName(
    _In_ ID3D12DeviceChild* resource,
    _In_z_ const wchar_t (&name)[TNameLength])
{
    ...
    size_t* bitSize)
{
    ...
    *header = hdr;
    auto offset =
        sizeof(uint32_t) + sizeof(DDS_HEADER) + (bDXT10Header ? sizeof(DDS_HEADER_DXT10) : 0);
    ...
    size_t* bitSize)
{
    ...
    // open the file
    ScopedHandle hFile(
        safe_handle(CreateFile2(fileName, GENERIC_READ, FILE_SHARE_READ, OPEN_EXISTING, nullptr)));
    ...
    _Out_opt_ size_t* outNumRows)
{
    ...
    else if (fmt == DXGI_FORMAT_NV11)
    {
        ...
        numRows = uint64_t(height) * 2u; // Direct3D makes this simplifying assumption, although it
...
DXGI_FORMAT GetDXGIFormat(const DDS_PIXELFORMAT& ddpf)
{
    if (ddpf.flags & DDS_RGB)
    {
        ...
        case 32:
            ...
            // No DXGI format maps to ISBITMASK(0x000000ff,0x0000ff00,0x00ff0000,0x00000000) aka
            // D3DFMT_X8B8G8R8
        ...
        case 16:
            ...
            // No 3:3:2, 3:3:2:8, or paletted DXGI formats aka D3DFMT_A8R3G3B2, D3DFMT_R3G3B2,
            // D3DFMT_P8, D3DFMT_A8P8, etc.
            break;
        }
    }
    else if (ddpf.flags & DDS_LUMINANCE)
    {
        if (8 == ddpf.RGBBitCount)
        {
            ...
            {
                return DXGI_FORMAT_R8G8_UNORM; // Some DDS writers assume the bitcount should be 8
                                               // instead of 16
            }
        }
    ...
    _Inout_ D3D12_SUBRESOURCE_DATA& res)
{
    ...
    case DXGI_FORMAT_P016:
        ...
        else
        {
            // Plane 1
            res.pData =
                reinterpret_cast<const uint8_t*>(res.pData) + uintptr_t(res.RowPitch) * height;
    ...
    _Out_opt_ bool* outIsCubeMap)
{
        ...
        (MAKEFOURCC('D', 'X', '1', '0') == header->ddspf.fourCC))
    {
        auto d3d10ext = reinterpret_cast<const DDS_HEADER_DXT10*>(
            reinterpret_cast<const uint8_t*>(header) + sizeof(DDS_HEADER));
    ...
    // Bound sizes (for security purposes we don't trust DDS file metadata larger than the Direct3D
    ...
    case D3D12_RESOURCE_DIMENSION_TEXTURE2D:
        if (isCubeMap)
        {
            ...
            (arraySize > D3D12_REQ_TEXTURE2D_ARRAY_AXIS_DIMENSION) ||
    ...
    if (header->ddspf.flags & DDS_FOURCC)
    {
        if (MAKEFOURCC('D', 'X', '1', '0') == header->ddspf.fourCC)
        {
            auto d3d10ext = reinterpret_cast<const DDS_HEADER_DXT10*>(
                reinterpret_cast<const uint8_t*>(header) + sizeof(DDS_HEADER));
            auto mode =
                static_cast<DDS_ALPHA_MODE>(d3d10ext->miscFlags2 & DDS_MISC_FLAGS2_ALPHA_MODE_MASK);
            switch (mode)
            {
            ...
            (MAKEFOURCC('D', 'X', 'T', '2') == header->ddspf.fourCC) ||
...
//--------------------------------------------------------------------------------------
void SetDebugTextureInfo(_In_z_ const wchar_t* fileName, _In_ ID3D12Resource** texture)
{
#if !defined(NO_D3D12_DEBUG_NAME) && (defined(_DEBUG) || defined(PROFILE))
    if (texture)
    {
...
//--------------------------------------------------------------------------------------
_Use_decl_annotations_ HRESULT DirectX::LoadDDSTextureFromMemory(
...
_Use_decl_annotations_ HRESULT DirectX::LoadDDSTextureFromMemoryEx(
    ID3D12Device* d3dDevice,
    const uint8_t* ddsData,
    size_t ddsDataSize,
    size_t maxsize,
...
//--------------------------------------------------------------------------------------
_Use_decl_annotations_ HRESULT DirectX::LoadDDSTextureFromFile(
    ID3D12Device* d3dDevice,
...
_Use_decl_annotations_ HRESULT DirectX::LoadDDSTextureFromFileEx(
    ID3D12Device* d3dDevice,
    const wchar_t* fileName,
    size_t maxsize,
    D3D12_RESOURCE_FLAGS resFlags,

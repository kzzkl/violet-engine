...
void button_page::initialize_sample_button()
{
    ...
    m_button->layout()->set_width(150.0f);
    m_button->layout()->set_height(40.0f);
    m_button->event()->on_mouse_press = [&, this](window::mouse_key key, int x, int y) -> bool {
        ++m_click_counter;
    ...
    m_button_text->layout()->set_margin(50.0f, ui::LAYOUT_EDGE_LEFT);
    ...
    display_2->layout()->set_flex_direction(ui::LAYOUT_FLEX_DIRECTION_ROW);
    display_2->layout()->set_align_items(ui::LAYOUT_ALIGN_CENTER);

    m_icon_button =
        std::make_unique<ui::icon_button>(0xEA22, ui.theme<ui::icon_button_theme>("dark"));
    m_icon_button->event()->on_mouse_press = [&, this](window::mouse_key key, int x, int y) -> bool {
    ...
    m_icon_button_text->layout()->set_margin(50.0f, ui::LAYOUT_EDGE_LEFT);
